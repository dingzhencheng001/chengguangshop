...
public class AppConveyController {
    @Autowired
    private AppConveyControllerService appConveyControllerService;

    @ApiOperation("获取交易订单列表")
    @RequestMapping(value = "/listAll", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult listAll() {
        List<AppConvey> appConvey = appConveyControllerService.listAll();
    ...
    public CommonResult<CommonPage<AppConvey>> getList(
        ...
        List<AppConvey> conveyList = appConveyControllerService.listConvey(appConvey, pageNum, pageSize);
    ...
    public CommonResult delete(@PathVariable("id") Long id) {
        int count = appConveyControllerService.deleteConveyById(id);
        if (count == 1) {
    ...
    public CommonResult update(@PathVariable("id") Long id,@RequestBody AppConvey appConvey) {
        ...
        int count = appConveyControllerService.updateConvey(id, appConvey);
        if (count == 1) {
    ...
    public CommonResult create(@RequestBody AppConvey appConvey) {
        CommonResult commonResult;
        int count = appConveyControllerService.createConvey(appConvey);
        if (count == 1) {
            commonResult = CommonResult.success(count);
        } else {
            commonResult = CommonResult.failed();
